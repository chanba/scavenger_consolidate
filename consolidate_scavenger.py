...
1) CSV format (header required):
   ...
   0,1,addr1q...
   1,1,addr1q...
   (external column is ignored; derivation uses ROLE=0)

2) Derivation:
   --numaddresses N   (derives N addresses on ROLE=0)
...
------------------------
- log.jsonl           : detailed JSON per donor (HTTP code, donation_id, response_raw, signature_hex)
- summary.csv         : one row per donor (index, address, status)
- signatures.csv      : (index, path, address, destination, signature_hex)
- job_summary.txt     : human-friendly summary
...
- If using --numaddresses: cardano-address in PATH
"""
...
class DonorRow:
    line_no: int
    index: int
    role: int   # always 0 in slim mode, but kept for clarity
...
class DonationResult:
    ...
    destination: str
    http_code: int
    status_class: str      # success | already_assigned | not_registered | client_error | server_error | rate_limited | other | dry_run | sign_error
...
def human_bucket(status_class: str, dry_run: bool, from_addr: str, to_addr: str) -> str:
    ...
    unassigned : success AND from==to (donate back to self)
    failed     : everything else (not_registered, client_error, server_error, rate_limited, other, sign_error)
    """
    if dry_run:
        return "unassigned"  # dry-run does not change assignment
...
def derive_addresses_external(mnemonic: str, account: int, n: int, network_tag: str) -> List[DonorRow]:
    ...
    for i in range(n):
        role = 0  # external only in slim mode
        # payment xpub at role/index
    ...
    with path.open(newline="", encoding="utf-8") as f:
        ...
        for line_no, rec in enumerate(reader, start=2):
            ...
            try:
                ...
                continue
            role = 0  # ignore CSV "external" in slim mode; always external
...
def cip8_sign(skey_path: Path, original_addr: str, dest_addr: str) -> Tuple[str, str]:
    ...
    int(sig_hex, 16); int(pubkey_hex, 16)  # sanity
...
def main():
    ap = argparse.ArgumentParser(description="Slim Scavenger consolidator: mnemonic + (csv or derive) -> donate_to.")
    ap.add_argument("--mnemonic", required=True, help="15- or 24-word mnemonic (quoted)")

    # NEW: print derived addresses only and exit
    ap.add_argument("--derive-address-only", action="store_true",
                    help="Only derive external addresses (ROLE=0) and print them (one per line). No signing, no API calls. Requires --numaddresses.")

    ap.add_argument("--destination-addr", help="Recipient addr1... (required unless --derive-address-only)")
    ...
    ap.add_argument("--network-tag", choices=["mainnet","testnet"], default="mainnet", help="For address derivation with cardano-address (default: mainnet)")
    ...
    ap.add_argument("--per-request-delay", type=float, default=1.0, help="Sleep seconds after each non-429 response (default: 1.0)")
    ap.add_argument("--max429", type=int, default=6, help="Max retries on HTTP 429 per address (default: 6)")
    ap.add_argument("--backoff-base", type=float, default=5.0, help="Base seconds for exponential backoff on 429 (default: 5.0)")
    ap.add_argument("--out-dir", default="consolidate-logs", help="Base output directory; a timestamped run subfolder will be created inside")
    ap.add_argument("--log-file", help="Custom JSONL log path (overrides run-folder log.jsonl)")
    ap.add_argument("--dry-run", action="store_true", help="Generate and log signatures, but DO NOT call the API")
    args = ap.parse_args()
    ...
    if not args.destination_addr:
        print("ERROR: --destination-addr is required (or use --derive-address-only)", file=sys.stderr)
        sys.exit(2)
    ...
    # Artifact paths (all contained inside this run folder)
    ...
    sig_w = csv.writer(sig_f); sig_w.writerow(["index","path","address","destination","signature_hex"])
        ...
        for row in donors:
            ...
            with tempfile.TemporaryDirectory(prefix="donate-to-") as tmpdir:
                ...
                try:
                    ...
                    signature_hex, pubkey_hex = cip8_sign(skey_path, row.address, dest_addr)
                except Exception as e:
                    print(f"  ERROR during derive/sign: {e}")
                    status_class = "sign_error"
                    result = DonationResult(
                        line_no=row.line_no, index=row.index, role=row.role, path=row.path,
                        address=row.address, destination=dest_addr,
                        http_code=0, status_class=status_class,
                        donation_id=None, solutions_consolidated=None,
                        signature_hex=None, response_raw=str(e),
                    )
                ...
                sig_w.writerow([row.index, row.path, row.address, dest_addr, signature_hex]); sig_f.flush()
                ...
                if args.dry_run:
                    ...
                    result = DonationResult(
                        line_no=row.line_no, index=row.index, role=row.role, path=row.path,
                        address=row.address, destination=dest_addr,
                        http_code=0, status_class="dry_run",
                        donation_id=None, solutions_consolidated=None,
                        signature_hex=signature_hex, response_raw="",
                    )
                    log_f.write(json.dumps(asdict(result)) + "\n"); log_f.flush()
                    results.append(result)
                    if args.per_request_delay > 0: time.sleep(args.per_request_delay)
                    continue
                ...
                while True:
                    try:
                        resp = donate_to(args.api_url, dest_addr, row.address, signature_hex, session=api_session)
                        http_code = resp.status_code; body_text = resp.text
                    except requests.RequestException as e:
                        http_code = 0; body_text = str(e)

                    if http_code == 429:
                        attempts_429 += 1
                        print(f"  HTTP 429 (Too Many Requests) attempt {attempts_429}/{args.max429}")
                        if attempts_429 >= args.max429:
                            break
                        retry_after = 0.0
                        try: retry_after = float(resp.headers.get("Retry-After", "0"))
                        except Exception: retry_after = 0.0
                        delay = retry_after if retry_after > 0 else args.backoff_base * (2 ** (attempts_429 - 1))
                        print(f"  Sleeping {delay:.1f}s before retry..."); time.sleep(delay); continue
                    else:
                        break
                ...
                if body_text:
                    try: resp_json = json.loads(body_text)
                    except Exception: resp_json = None

                status_class = classify_status(http_code)
                if resp_json and isinstance(resp_json, dict):
                    ...
                    try: solutions_consolidated = int(val) if val is not None else None
                    except Exception: solutions_consolidated = None

                print(f"  HTTP status: {http_code} -> {status_class}")
                if donation_id: print(f"  donation_id: {donation_id}")
                if solutions_consolidated is not None: print(f"  solutions_consolidated: {solutions_consolidated}")
                if body_text: print(f"  resp: {body_text[:400]}")

                result = DonationResult(
                    line_no=row.line_no, index=row.index, role=row.role, path=row.path,
                    address=row.address, destination=dest_addr,
                    http_code=http_code, status_class=status_class,
                    donation_id=donation_id, solutions_consolidated=solutions_consolidated,
                    signature_hex=signature_hex, response_raw=body_text,
                )
                log_f.write(json.dumps(asdict(result)) + "\n"); log_f.flush()
                results.append(result)
    ...
    with summary_csv_path.open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["index","path","address_from","address_to","http_code","status_class","donation_id","solutions_consolidated"])
        for r in results:
            w.writerow([r.index, r.path, r.address, r.destination, r.http_code, r.status_class, r.donation_id or "", r.solutions_consolidated or ""])

    # Write human-readable job_summary.txt
    assigned = 0; failed = 0; unassigned = 0
    lines: List[str] = []
    ...
    for r in results:
        ...
        elif bucket == "unassigned": unassigned += 1
        else: failed += 1
        if bucket == "assigned":
    ...
    lines.append(f"- log.jsonl        : {log_path.name}")
    lines.append(f"- summary.csv      : {summary_csv_path.name}")
    lines.append(f"- signatures.csv   : {signatures_csv_path.name}")
